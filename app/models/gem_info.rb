...
class GemInfo
  ...
  def self.compact_index_versions(date, version: 1)
    ...
    query = ["(SELECT r.name, v.created_at as date, v.#{checksum_column} as info_checksum, v.number, v.platform, v.sha256, v.ruby_abi
              FROM rubygems AS r, versions AS v
              WHERE v.rubygem_id = r.id AND
                    v.created_at > ?)
              UNION
              (SELECT r.name, v.yanked_at as date, v.#{yanked_checksum_column} as info_checksum, '-'||v.number, v.platform, v.sha256, v.ruby_abi
              FROM rubygems AS r, versions AS v
              WHERE v.rubygem_id = r.id AND
  ...
  def self.compact_index_public_versions(updated_at, version: 1)
    ...
    query = ["SELECT r.name, v.indexed, COALESCE(v.yanked_at, v.created_at) as stamp,
                     ...
                     v.number, v.platform, v.ruby_abi
              FROM rubygems AS r, versions AS v
              WHERE v.rubygem_id = r.id AND
  ...
  def self.map_gem_versions(versions_by_gem, version: 1)
    klass = VERSIONS.fetch(version)[:klass]
    versions_by_gem.map do |gem_name, versions|
      compact_index_versions = versions.map do |v|
        # ruby_abi lets skinny (content-addressable) binaries get a content address
        # (number-<sha8>), which needs the hex checksum. slice keeps only the members
        # the struct defines, so v1 (no ruby_abi) just ignores it.
        args = { number: v["number"], platform: v["platform"],
                 checksum: Version._sha256_hex(v["sha256"]), info_checksum: v["info_checksum"],
                 ruby_abi: v["ruby_abi"] }
        klass.new(**args.slice(*klass.members))
      end
  ...
  def compute_compact_index_info(version:)
    requirements_and_dependencies.map do |row|
      ...
      number, platform, checksum, info_checksum, ruby_version, rubygems_version, created_at, ruby_abi, = row
      version_class = VERSIONS.dig(version, :klass)
      checksum = Version._sha256_hex(checksum)
      created_at = created_at&.utc&.iso8601
      args = { number:, platform:, checksum:, info_checksum:, dependencies:, ruby_version:, rubygems_version:, created_at:, ruby_abi: }
  ...
  def fetch_requirements_and_dependencies
    group_by_columns = "number, platform, sha256, info_checksum, required_ruby_version, required_rubygems_version, versions.created_at, ruby_abi"
